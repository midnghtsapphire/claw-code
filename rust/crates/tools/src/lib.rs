...
mod tests {
  ...
  "nbformat_minor": 5
    ...
    fn repl_executes_python_code() {
        ...
        // Skip gracefully when Python is not present in the CI environment rather
        // than panicking — the "runtime not found" error is the expected signal.
        let result = match result {
            Err(ref e) if e.contains("runtime not found") => {
                eprintln!("SKIP repl_executes_python_code: {e}");
                return;
            }
